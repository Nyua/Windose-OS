...
- Side Fill: Mirror side-bar assets from the sidebars folder at full size (default). Alternative pattern is optional via settings.
...
Use Redux Toolkit for the global store. Track the following:
...
- Start Menu Actions: Restart refreshes the page; Shut Down attempts to close the page/tab (fallback to in-app exit screen if blocked).
...
| sfxClickPath | string | "" | Mouse click SFX file path (repo-relative) |
| sfxNotifyPath | string | "" | Notification chime SFX file path (repo-relative) |
...
| TC-016 | Minimize animation | Minimize window | Smooth transition to tab | Rapid minimize/restore |
...
| --- | --- | --- | --- | --- | --- |
| YouTube embed blocked | iframe error or blocked event | Show in-window error banner | Link-out button | WARN | No |
| Twitter embed blocked | embed load timeout | Show placeholder timeline | Link-out button | WARN | No |
...
- SFX events: mouse clicks and notification chimes only (no other events).
...
- Sound file paths are editable in the password-protected Settings.
