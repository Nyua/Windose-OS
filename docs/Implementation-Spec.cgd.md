...
points-passed: 1-8
document-sha256: 43f44425d93a0cecd727cc3a56e2d95a8b533a85aa1103d74a400cc6dda56ad3
hitl-claims:
  - id: claim-75a1a76c
    text: "Placeholder sprites are in /public/buttons."
    value: "/public/buttons"
    source: "PENDING: verify repo path and file list"
    location: "phase2-notes/2"
    round: B
  - id: claim-85674d26
    text: "Assets live in public/bootscreen: boot_bios.png, bios_logo.png, bootscreen_4_3.png, setup.png."
    value: "public/bootscreen assets list"
    source: "PENDING: verify repo path and file list"
    location: "boot-sequence/assets"
    round: B
  - id: claim-8da2a7ec
    text: "src/App.vue: Removed unused reason parameter from ensureWebcamOpen and scheduleWebcamReopen."
    value: "TS fix in src/App.vue"
    source: "PENDING: verify file diff or commit"
    location: "recent-fixes/1"
    round: B
  - id: claim-300130e8
    text: "src/components/Desktop.vue: Fixed property access for viewportScale (changed to props.viewportScale)."
    value: "TS fix in src/components/Desktop.vue"
    source: "PENDING: verify file diff or commit"
    location: "recent-fixes/2"
    round: B
  - id: claim-31dc4419
    text: "src/components/DesktopIcon.vue: Added missing viewportScale to defineProps interface."
    value: "TS fix in src/components/DesktopIcon.vue"
    source: "PENDING: verify file diff or commit"
    location: "recent-fixes/3"
    round: B
---
...
- ASSUMPTION: Desktop Wallpaper uses public/background/bg.png sourced from Sprites-NSO/background/bg.png.
...
- ASSUMPTION: Fonts are loaded from local files in the repo (no CDN).

### 1.4 Art Assets
ASSUMPTION: All sprite/image assets are in the `Sprites-NSO/` folder, organized by category.
...
- TARGET (Future UI): `references-for-design-doc\Stream.png` - Stream topic list reference.
...
- ASSUMPTION: **Location:** `src/stores/time.ts`
...
- NIGHT: Dark purple/blue tint. TARGET: Streaming is ONLY unlocked at Night. Reference image attached: `references-for-design-doc\Night.png`
...
- TARGET: Placeholder apps in Phase 1 are Internet (empty window) and Stream (video embed placeholder).
- Boot sequence overlay: BIOS -> Boot Menu -> Startup (first visit + Start Menu triggers).
- TARGET: Basic assets are hooked up from Sprites-NSO/ and reference images for layout checks.

### Phase 2 Notes (Concrete)
- TARGET: Load all fonts from local repo files.
- TARGET: Use code-based window buttons for now; switch to WacOS sprite-based focused/unfocused states when assets are sourced. UNVERIFIED: Placeholder sprites are in /public/buttons (verify repo path).
- No hover states on window buttons.
- Bevel borders are 2px.
- Side assets from sidebars folder are full size.
- TARGET: Start Menu uses NSO sprites (ASSUMPTION: start-menu/start_menu.png, start_button.png, start_pressed.png exist). Start button shows pressed sprite while menu is open.
- No scanline effect in Phase 2.

### 3.1 Desktop and Taskbar
Taskbar (bottom of screen):
- TARGET: Taskbar background color matches the Start button base color (ASSUMPTION: #D3C1DE from start_button.png).
...
- TARGET: Start Menu Actions are Restart -> play boot sequence then refresh page, and Shut Down -> play boot sequence then attempt to close the tab.
...
- TARGET: Quick Launch uses sprite buttons from public/quickmenu (ASSUMPTION: button_tweeter.png, button_jine.png, button_task-manager.png exist).
...
- TARGET: Secret.txt is placed slightly right of center on the desktop (custom default position).
...
### 3.1.3 Task Manager
- TARGET: Basic Task Manager UI replaces the placeholder.
...
ASSUMPTION: Reference image for complete webcam is `references-for-design-doc\Webcam.png`
...
TARGET (Deferred): Future UI reference is `references-for-design-doc\Stream.png` (Metube topic list).
...
- TARGET (Future): Read label becomes clickable to show avatars of users who have viewed the message (requires server/user accounts).
- TARGET: Centered day pill label uses a Noon placeholder in Phase 1.
...
- Sticker menu is a collapsible drawer above the text area.
  ...
  - TARGET: Sticker grid uses Sprites-NSO sticker images (ASSUMPTION: assets exist; see StickerMenu.png for layout).
...
- TARGET: Chatroom (Hybrid) in Phase 1 uses local-only storage; messages are per-user in local storage and only visible on the same device/browser. Not live - on page refresh, a notification pop-up (bottom-right of the interactive area) shows a shortened preview of new messages. Clicking the notification opens the chat. TARGET (Phase 2): add server-backed storage for cross-user visibility.

Reference images: `references-for-design-doc\Jine.png`, `references-for-design-doc\StickerMenu.png`

### 3.6 Tweeter
TARGET: Phase 1 embeds an iframe to https://x.com/ProbablyLaced (placeholder). TARGET (Future): fully usable in-app Twitter clone.
- If the embed fails to load within 4 seconds, show a placeholder with an external link.

### 3.7 Internet
TARGET: No functionality in Phase 1. Create an empty application component as a placeholder.
...
- TARGET: Password is fixed to "angelkawaii2" in Phase 1 and not editable in the UI.
...
TARGET: TimeSlot debug control in Control Panel provides a Default/Noon/Dusk/Night toggle group that writes to timeSlotOverrideEnabled/timeSlotOverride.

#### 3.8.3 Persistence
- TARGET: Settings persist in local storage (per-browser).
...
- TARGET: On click, prompt for confirmation; if confirmed, clear `localStorage` and reload the page.

### 3.9 Boot Sequence (Startup/BIOS/Boot Menu)
- TARGET: Boot sequence plays on first visit only (local storage flag).
...
- TARGET: Default timings are Black 2000ms, BIOS 5000ms, bootscreen fade 2000ms (all configurable via settings).
...
- UNVERIFIED: Assets live in public/bootscreen: boot_bios.png, bios_logo.png, bootscreen_4_3.png, setup.png (verify repo path).
...
- TARGET (Future): add boot sound effects and transition animations.

## 3.10 Future QOL Backlog
- TARGET (Backlog): Time debug expansion adds toggles for dayOfYear/clock display and an optional on-screen readout to validate time-driven state transitions.
...
- TARGET: Stats include none beyond system-based day/time in this phase.
- TARGET: Day/time come from system date/time only; no manual Next Day logic.
- TARGET: No Next Day button or stub in Phase 3.




## Phase 5 Notes (Concrete)
- TARGET: SFX events include mouse clicks, notifications, JINE send, and window operations (open/close/minimize/restore/fullscreen/drag start).
- TARGET: Volume control is in the taskbar; default volume is configurable.
- TARGET: CRT scanline overlay is toggleable in Settings and adjustable for intensity.
- TARGET: Sound file paths are editable in the password-protected Settings (public/sounds).
...
- **`src/App.vue`**: UNVERIFIED (reported 2026-02-04): Removed unused `reason` parameter from `ensureWebcamOpen` and `scheduleWebcamReopen`.
- **`src/components/Desktop.vue`**: UNVERIFIED (reported 2026-02-04): Fixed property access for `viewportScale` (changed to `props.viewportScale`).
- **`src/components/DesktopIcon.vue`**: UNVERIFIED (reported 2026-02-04): Added missing `viewportScale` to `defineProps` interface.
...
| --- | --- | --- | --- | --- |
| 1 | Placeholder sprites are in /public/buttons. | PENDING |  |  |
| 2 | Assets live in public/bootscreen: boot_bios.png, bios_logo.png, bootscreen_4_3.png, setup.png. | PENDING |  |  |
| 3 | src/App.vue: Removed unused reason parameter from ensureWebcamOpen and scheduleWebcamReopen. | PENDING |  |  |
| 4 | src/components/Desktop.vue: Fixed property access for viewportScale (changed to props.viewportScale). | PENDING |  |  |
| 5 | src/components/DesktopIcon.vue: Added missing viewportScale to defineProps interface. | PENDING |  |  |
